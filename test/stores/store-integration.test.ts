...
describe("Store Integration", () => {
  ...
  describe("Project and Session Lifecycle", () => {
    test("should handle complete project creation and session setup", async () => {
      ...
      await useSessionsStore.getState().loadSessions("new-project", "/test/path")
      ...
      expect(sessionState.sessions.get("new-project")).toEqual(mockSessions)
    })

    test("should handle project removal and session cleanup", async () => {
      ...
      // Clear sessions for removed project
    ...
    test("should handle project selection and session loading", async () => {
      ...
      const [,] = await Promise.all([
        ...
        useSessionsStore.getState().loadSessions("selected-project", "/test/path"),
      ])
  ...
  describe("Cross-Store State Consistency", () => {
    test("should maintain consistency when project is updated", async () => {
    ...
    test("should handle concurrent operations across stores", async () => {
    ...
    test("should handle error propagation between stores", async () => {
  ...
  describe("Memory Management and Cleanup", () => {
    test("should handle large datasets efficiently", async () => {
      ...
      for (let i = 0; i < 100; i++) {
        const sessions = Array.from({ length: 10 }, (_, j) => ({
          id: `session-${i}-${j}`,
          projectID: `project-${i}`,
          directory: `/path/${i}`,
          title: `Session ${j}`,
  ...
  describe("Store Persistence Integration", () => {
    test("should handle hydration of persisted project data", () => {
      const persistedProject = TestDataFactory.createProject({
      ...
      useProjectsStore.setState({
        ...
        loading: false,
        error: null,
        instanceOperations: {}, // This should not be persisted
      })
    ...
    test("should handle session state without persistence", () => {
      // Sessions are not persisted, so they should start empty
    ...
    test("should handle mixed persisted and non-persisted state", () => {
      const project = TestDataFactory.createProject({ id: "mixed-project" })

      // Set persisted project state
      useProjectsStore.setState({
        projects: [project],
        currentProject: project,
        loading: false, // Should be reset on hydration
        error: null,
        instanceOperations: {},
      })

      // Set non-persisted session state
      useSessionsStore.setState((state) => {
        state.sessions.set("mixed-project", [
          {
            id: "session-1",
      ...
      expect(projectState.projects).toContain(project)
      expect(sessionState.sessions.get("mixed-project")).toBeDefined()
    })
  })

...
import {
  useProjectsActions,
  useCurrentProject as useCurrentProjectFromStore,
...
export default function Terminal() {
  ...
  useEffect(() => {
    ...
    const handleKey = (e: { key: string; domEvent: KeyboardEvent }) => {
      ...
      if (ev.key === "ArrowUp" || ev.key === "ArrowDown") {
        ...
        if (ev.key === "ArrowUp")
          nextIndex = cursor < 0 ? history.length - 1 : Math.max(0, cursor - 1)
        if (ev.key === "ArrowDown")
          nextIndex = cursor < 0 ? -1 : Math.min(history.length - 1, cursor + 1)
  ...
  const containerClass = fullScreen ? "fixed inset-0 z-50 bg-background p-3" : ""

  const { startInstance } = useProjectsActions()
  const storeProject = useCurrentProjectFromStore()
  const instanceRunning = Boolean(
    storeProject?.instance && storeProject.instance.status === "running"
  )
  ...
  return (
    <div className={cn("p-4", containerClass)}>
      <div className="mb-3 space-y-1">
        <h1 className="text-lg font-semibold">Terminal</h1>
        <p className="text-muted-foreground text-sm">
          Type directly in the terminal. Use Ctrl+C to kill.
        </p>
        <p className="text-muted-foreground text-xs">
          Worktree: <span className="font-medium">{worktreeTitle}</span>
        </p>
        <p className="text-muted-foreground font-mono text-xs">
          {workingPath ?? "(path unavailable)"}
        </p>
      </div>
      <Card className="rounded-md border bg-black">
        <div className="flex items-center justify-between border-b p-2">
          <div className="text-muted-foreground text-xs">Session: {sessionId ?? "(new)"}</div>
          <div className="flex gap-2">
      ...
      <div className="text-muted-foreground mt-2 flex flex-wrap items-center gap-2 text-xs">
        ...
        <span className="text-muted-foreground/70">({workingPath || ""})</span>
